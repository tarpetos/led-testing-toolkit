...
function createAndPositionLeds(ledsData) {
  ...
  const cornerPositions = [
    { x: 0, y: 0 },
    { x: containerWidth - uniformItemWidth, y: 0 },
    { x: containerWidth - uniformItemWidth, y: containerHeight - uniformItemHeight },
    { x: 0, y: containerHeight - uniformItemHeight },
  ];
  ...
  ledElements.forEach((ledItem, index) => {
    ...
    if (index < 4 && index < N) {
      xPos = cornerPositions[index].x;
      yPos = cornerPositions[index].y;
    } else {
      const gridStartX = uniformItemWidth;
      const gridStartY = uniformItemHeight;
      const gridWidth = containerWidth - (2 * uniformItemWidth);
      const itemsPerRow = Math.max(1, Math.floor(gridWidth / uniformItemWidth));
      const gridIndex = index - 4;
      const row = Math.floor(gridIndex / itemsPerRow);
      let col = gridIndex % itemsPerRow;

      if (row % 2 !== 0) {
        col = itemsPerRow - 1 - col;
      }

      xPos = gridStartX + col * uniformItemWidth;
      yPos = gridStartY + row * uniformItemHeight;
    }
...
function togglePlay() { controlPlayer(currentPlayerStatus.is_playing ? 'pause' : 'resume'); }
function stopPlayback() { controlPlayer('stop'); }
