...
async def test_generate_etalons_func():
    with (
        mock.patch("led_testing_toolkit.scripts.generate_etalons.logger") as mock_log,
...
async def test_generate_etalons_main():
    with (
        mock.patch("led_testing_toolkit.scripts.generate_etalons._get_collections") as mock_get,
...
async def test_main():
    with (
        ...
        mock.patch(
            "led_testing_toolkit.scripts.generate_etalons.generate_etalons_main", new_callable=mock.AsyncMock
        ) as mock_gen,
    ):
