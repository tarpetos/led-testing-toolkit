...
async def test_extract_led_rgb_data():
    raw_data = {
        "LED1": [[0.1, [255, 0, 0], 1.0], [0.2, [0, 255, 0], 2.0]],
        "LED2": "invalid",
        "LED3": [
...
def test_compute_average_abs_times():
    records = [mock.Mock(coordinates=[mock.Mock(z=1.0), mock.Mock(z=2.0)]), mock.Mock(coordinates=[mock.Mock(z=3.0)])]
...
async def test_generate_etalon():
    with (
        mock.patch("led_testing_toolkit.utils.data_processing.read_measured") as mock_read,
        mock.patch("led_testing_toolkit.utils.data_processing.MongoDbConnector") as mock_conn,
...
def test_convert_normalized_to_raw_format():
    ...
    norm = {
        ...
        "LED2": {"r": [mock.Mock(coordinates=[Point(x=1, y=255, z=0)])]},
    }
...
async def test_save_patterns_to_db():
    with (
        mock.patch("led_testing_toolkit.utils.data_processing.logger") as mock_log,
